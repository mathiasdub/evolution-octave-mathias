...
@ray.remote
def evaluate_agent_remote_pos(genes, cfg, max_steps=500):
    env = EvoGymEnv(cfg["env_name"], cfg["robot"])
    agent = Agent(Network, cfg, genes=genes)
    fitness = evaluate(agent, env, max_steps)
    return fitness  
...
def ES(config):
    ...
    for gen in bar:
        ...
        tasks = [evaluate_agent_remote_pos.remote(a.genes, cfg) for a in population]    
...
@ray.remote
def evaluate_agent_remote(genes, cfg, max_steps=500):
    env = EvoGymEnv(cfg["env_name"], cfg["robot"])
    agent = Agent(Network, cfg, genes=genes)
    fitness = evaluate(agent, env, max_steps)
...
def CMA_ES(config):
    ...
    bar = tqdm(range(cfg["generations"]))
    for gen in bar:
        steps=min(max_max_steps, min_max_steps + 4*gen * (max_max_steps - min_max_steps) // cfg["generations"])
        print(steps)
        solutions = es.ask()

        # Évaluation parallèle avec Ray
        tasks = [evaluate_agent_remote.remote(genes, cfg, steps) for genes in solutions]
...
def MAP_Elites(config):
    ...
    archive = GridArchive(
        solution_dim=len(Agent(Network, cfg).genes),  # ou une valeur connue si elle est fixe
        dims=[10, 10],
        ranges=[[-1, 1], [-1, 1]],
        seed=42
    )
    ...
    for gen in bar:
        ...
        # Parallel evaluation
        futures = [evaluate_remote.remote(genes, cfg) for genes in all_solutions]
        results = ray.get(futures)
        fitnesses, descriptors = zip(*results)

        # Tell results back to emitters
        idx = 0
        for emitter in emitters:
            batch = emitter.batch_size
            emitter.tell(
                solutions=all_solutions[idx:idx + batch],
                objective_values=fitnesses[idx:idx + batch],
                behavior_values=descriptors[idx:idx + batch]
            )
            idx += batch

        best_fit = max(fitnesses)
        fits.append(best_fit)
        total_evals.append(len(all_solutions) * (gen + 1))
    ...
    objective_values = archive.as_pandas().objective
    best_idx = np.argmax(objective_values)
    best_solution = archive.as_pandas().solution.iloc[best_idx]
    best_fitness = objective_values.iloc[best_idx]

    best_agent = Agent(Network, cfg, genes=best_solution.values)
