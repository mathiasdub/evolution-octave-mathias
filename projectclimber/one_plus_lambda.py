...
# Initialiser Ray (si déjà lancé, ça ne plante pas)
ray.init(ignore_reinit_error=True)

# Fonction d'évaluation distante
@ray.remote
def evaluate_remote(genes, config):
    sim = Evosim(config)
    agent = Agent(Network, config, genes=genes)
    fitness = evaluate(agent, sim.env, max_steps=config["max_steps"])
...
def one_plus_lambda(config):
    ...
    for gen in bar:
        # Génération de gènes mutants
        gene_list = [elite.mutate_ga() for _ in range(cfg["lambda"])]

        # Evaluation parallèle
        futures = [evaluate_remote.remote(genes, cfg) for genes in gene_list]
        results = ray.get(futures)

        # Sélection du meilleur
        pop_fitness = [fit for fit, _ in results]
        best = np.argmax(pop_fitness)
        best_fit, best_genes = results[best]

        # Mise à jour de l'élite
        if best_fit > elite.fitness:
            elite.genes = best_genes
    ...
    plt.ylabel("Fitness")
    plt.title("Évolution du fitness (1+λ)")
    plt.show()
